...
def show_samples(dataloader, model, device, num_correct=3, num_incorrect=3):
    ...
    correct_images, correct_labels, correct_preds, incorrect_images, incorrect_labels, incorrect_preds = find_images(dataloader,
                                                                                                                      model,
                                                                                                                      device,
                                                                                                                      num_correct,
                                                                                                                     num_incorrect)
    # Display the results in a grid
    fig, axes = plt.subplots(num_correct + num_incorrect, 1, figsize=(10, (num_correct + num_incorrect) * 5))

    for i in range(num_correct):
        axes[i].imshow(correct_images[i].permute(1, 2, 0))
        axes[i].set_title(f'Correctly Classified: True Label = {correct_labels[i]}, Predicted = {correct_preds[i]}')
        axes[i].axis('off')

    for i in range(num_incorrect):
        axes[num_correct + i].imshow(incorrect_images[i].permute(1, 2, 0))
        axes[num_correct + i].set_title(f'Incorrectly Classified: True Label = {incorrect_labels[i]}, Predicted = {incorrect_preds[i]}')
...
def batch_predict(model, images, device):
    model.eval()
    batch = torch.stack(tuple(preprocess_image(image, (224, 224)) for image in images), dim=0)
